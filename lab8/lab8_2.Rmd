...
```{r}
pivot_longer(expenditures, -expenditure, names_to="year", values_to="bn_dollars")
```

2. Restrict the data to medical and health expenditures only. Sort in ascending order.
```{r}
#expenditures %>%
  #select(expenditure == "Medical and Health")
```
...
heartrate2 %>% 
  #separate(patient, into= c("patient", "sex"), sep = "_") %>%
  #pivot_longer(-patient, names_to = "patient", values_to = "info")
```
